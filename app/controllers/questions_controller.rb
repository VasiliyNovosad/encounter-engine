class QuestionsController < ApplicationController
  ...
  def new
    ...
    @question = questions.build(name: "Сектор #{questions.size + 1}")
  ...
  def create_batch
    ...
    question_params[:answers_list].split(/\n+/).each do |answers|
      ...
      question = questions.build(name: "#{question_params[:name]} #{questions.size + 1}", team_id: team_id)
  ...
  def copy
    questions_count = @level.questions.size
  ...
  def copy_to_bonus
    ...
    new_bonus = bonuses.build(name: "Бонус #{bonuses.size + 1}")
    new_bonus.set_list_position(bonuses.size + 1)
    @question.answers.each do |answer|
