  ...
  def show_short_log
    ...
    @levels.each do |level|
      @level_logs << @teams.map do |team|
        ...
        team_log = (team_logs.count > 0 && GamePassing.of_game(@game).of_team(team).first && GamePassing.of_game(@game).of_team(team).first.closed_levels.include?(level.id)) ? team_logs.last : nil # && GamePassing.of_game(@game).of_team(team).first && GamePassing.of_game(@game).of_team(team).first.closed_levels.include?(level.id)
